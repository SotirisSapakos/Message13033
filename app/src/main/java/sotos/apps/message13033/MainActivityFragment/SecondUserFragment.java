...
public class SecondUserFragment extends Fragment {
    ...
    private void initData() {
        if (preferences.getBoolean(Constants.PREFERENCES_USER_SAVE_STATS2, false)) {
            textName.setText(preferences.getString(Constants.PREFERENCES_USER_NAME2, ""));
            textAddress.setText(preferences.getString(Constants.PREFERENCES_ADDRESS2, ""));
        }
    }
    ...
    private void saveData() {
        if (validateTextInputs()) {
            ...
            editor.putString(Constants.PREFERENCES_USER_NAME2,
                    Objects.requireNonNull(textName.getText()).toString());
            editor.putString(Constants.PREFERENCES_ADDRESS2,
                    Objects.requireNonNull(textAddress.getText()).toString());
            editor.putBoolean(Constants.PREFERENCES_USER_SAVE_STATS2, true);
            editor.commit();
            alertSaveStats();
        }
    }
    ...
    private void alertSaveStats() {
        ...
        Drawable image = getResources().getDrawable(R.drawable.user_2_red);

        title.setText("Αποθήκευση στοιχείων");
        message.setText(preferences.getString(Constants.PREFERENCES_USER_NAME2, ""));
        address.setText(preferences.getString(Constants.PREFERENCES_ADDRESS2, ""));
